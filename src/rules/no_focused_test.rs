...
use crate::rules::{NO_FOCUSED_TEST_RULE_ID, Violation};
use crate::syntax::LineIndex;

const MESSAGE: &str = "Disallow focused tests (describe.only, it.only, test.only).";

const FOCUSED_TEST_NAMES: &[&str] = &["describe", "it", "test"];
...
pub fn check_file(
    ...
    let mut visitor = FocusedTestVisitor {
        violations: Vec::new(),
        file,
        line_index,
...
impl<'a, 'f> Visit<'a> for FocusedTestVisitor<'a, 'f> {
    fn visit_call_expression(&mut self, call: &CallExpression<'a>) {
        if let Expression::StaticMemberExpression(member) = &call.callee
            && let Expression::Identifier(object) = &member.object
            && FOCUSED_TEST_NAMES.contains(&object.name.as_str())
            && member.property.name == "only"
        {
            let pos = self.line_index.position_for(member.span);
            let subject = format!("{}.only", object.name);
