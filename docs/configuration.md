...
The default config generated by `niteo init` (without `--preset`) enables the current rule set. Most rules default to `warn`. `no-empty-interface` defaults to `error`, and `prefer-satisfies` defaults to `info`.
